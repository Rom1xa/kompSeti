...
static void *handle_client(void *arg)
{
    ...
    if (msg.type != MSG_AUTH || strlen(msg.payload) == 0) {
        ...
        osi_log(6, "Presentation", "serialize Message");
        osi_log(4, "Transport",    "send()");
        msg_send(sock, MSG_ERROR, "Authentication required: send MSG_AUTH with your nickname");
        close(sock);
        client_remove(sock);
        return NULL;
    }
    ...
    while (1) {
        ...
        case MSG_PRIVATE: {
            ...
            snprintf(buf, sizeof(buf), "[PRIVATE][%s]: %s", sender_nick, private_text);
            ...
            snprintf(buf, sizeof(buf), "[PRIVATE → %s]: %s", target_nick, private_text);
